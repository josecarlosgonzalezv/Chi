...
export default class DataTable extends Vue {
  ...
  _head() {
    ...
    heads.forEach((column: string | DataTableColumn, cellIndex: number) => {
      ...
      if (this.dataTableData.head[columnIndex].sortable && !this._sortable) {
        this._sortable = true;
      }
    });
  ...
  _calculateNumberOfPages() {
    ...
    const bodyLength = this._showSelectedOnly ? this._getSelectedFirstLevelRowsCount() : this.dataTableData.body.length;
  ...
  _pagination() {
    ...
    if (
      ...
      (this.dataTableData.body.length === 0 && this.mode === DataTableModes.CLIENT)
    ) {
      return null;
    } else {
      return (
